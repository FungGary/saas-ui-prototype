      ...
      <ul>
        ...
        >
          <div :class="setColumnsAsidelayout" v-if="!v.isLink || (v.isLink && v.isIframe)">
            <i :class="getMenuIconClass(v.icon)"></i>
            <div class="font14">
              {{
...
import { getMenuIconClass } from '@/utils/icon';
...
export default {
  ...
  methods: {
    ...
    filterRoutesFun(arr) {
      return arr
        .filter((item) => item.path && item.path !== '/admin/order/bag' && item.path !== 'bag' && item.hidden !== true && item.is_show !== 0 && !(item.meta && item.meta.isHide))
        .map((item) => {
