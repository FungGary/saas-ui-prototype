  ...
  <div class="layout-navbars-breadcrumb">
    ...
    <el-breadcrumb class="layout-navbars-breadcrumb-hide" v-if="isShowcrumb" :style="{ display: isShowBreadcrumb }">
      <transition-group name="breadcrumb" mode="out-in">
        <el-breadcrumb-item v-for="(v, k) in [...breadCrumbList, ...crumbPast]" :key="v.path + '-' + k">
...
export default {
  ...
  computed: {
    ...
    crumbPast() {
      ...
      let allMenuList = R(menuList, []);
      let selectMenu = [];
      const existingPaths = this.breadCrumbList.map(item => item.path);
      if (existingPaths.includes(that.$route.path)) return selectMenu;
      if (allMenuList.length > 0) {
        allMenuList.forEach((a) => {
